<?php
#assignment 1:
$a = 100;
$b = 200;
$c = 100;
if($a<$b && $a===$c && $b=$a+$c){
    echo "Yes";
}

echo "<br>";

#assignment 2:
// Test Case 1
$a = 100;
$b = 200;
$c = 300;
if($a<$b && $a<$c)
    echo "A Is Not Larger Than B Or C<br>";
elseif($a>$b)
    echo "A Is Larger Than B<br>";
elseif($a>$c)
    echo "A Is Larger Than C<br>";
?>
<?php
#assignment 3:
$admins = ["Osama", "Ahmed", "Sayed"];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $username = $_POST['user'] ?? '';
    echo "The Request Method Is Post And Username Is $username<br>";
    //if he is an admin
    if (in_array($username, $admins)) {
        echo "This Username $username Is Admin<br>";
    }
}
?>
<!--The Form-->
<form action="" method="POST">
  <input type="text" name="user">
  <input type="submit" value="Send">
</form>

<?php
#assignment 4:
$a = 30;
$b = 20;
$c = 10;
echo ((($a + $b === $c)? "A + B = C": (($a + $c === $b)? "A + C = B": (($b + $c === $a)? "B + C = A": "The End"))));  //"B + C = A"

echo "<br>";

#assignment 5:
$name = "Osama";
$age = 40;
$country = "Egypt";
if ($age > 18 && gettype($name) === "string" && $country === "Egypt") {
  echo nl2br("The Age Is Good To Go
              The Name Is Good To Go
              The Country Is Good To Go<br>");

}

#assignment 6:
$genre = "Hack And Slash";
switch($genre) {
    case "RPG":
        echo "I Recommend Ys Games";
        break;
    case "Hack And Slash":
        echo "I Recommend Castlevania Games";
        break;
    case "FPS":
        echo "I Recommend Uncharted Games";
        break;
    case "Platform":
        echo "I Recommend Megaman Games";
        break;
    case "Puzzle":
        echo "I Recommend Megaman Games";
        break;
    default:    
        echo "I Recommend Shadow Of Mordor And Shadow Of War";
}

echo "<br>";

#assignment 7:
$num_one = 23;
$num_two = 5;
$op = "+";
switch($op) {
    case "+":
        echo $num_one + $num_two."<br>";  //28
        break;
    case "-":
        echo $num_one - $num_two."<br>";  //18
        break;
    case "*":
        echo $num_one * $num_two."<br>";  //115
        break;
    case "/":
        echo $num_one / $num_two."<br>";  //4
        echo $num_one % $num_two."<br>";  //3
        break;
    default:    
        echo "Unknown Operation<br>";
}

#assignment 8:
$day = "Sat";
if($day=="Sat" || $day=="Sun" || $day=="Mon")
    echo "We Are Open All The Day<br>";
elseif($day=="Tue" || $day=="Wed")
    echo "We Are Open From 08:12<br>";
elseif($day=="Thu" || $day=="Fri")
    echo "We Are Closed<br>";
else
    echo "Unknown Day<br>";
